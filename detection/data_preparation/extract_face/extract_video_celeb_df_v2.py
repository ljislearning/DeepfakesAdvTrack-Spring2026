...
def has_pngs(folder):
    return os.path.isdir(folder) and any(name.endswith(".png") for name in os.listdir(folder))
    ...
    for idx in tqdm(range(len(real_video_name))):
        ...
        for a in range(0, id_num+1):
            ...
            if os.path.isfile(fake_video_path):
                expected_fake_paths.append(os.path.join(image_root_path, 'Celeb-synthesis', fake_video_name))
        if has_pngs(output_path) and all(has_pngs(path) for path in expected_fake_paths):
            continue
